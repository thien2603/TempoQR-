...
from src.core.utils import print_info, getAllDicts, loadTkbcModel, loadTkbcModel_complex
...
parser.add_argument('--save_to', default='tempoqr_paper', type=str,
...
def eval(qa_model, dataset, batch_size = 128, split='valid', k=10):
    ...
    eval_log = []
    k_for_reporting = k
    k_list = [1,10]  # Đánh giá Hits@1 và Hits@10
    max_k = max(k_list)
...
def train(qa_model, dataset, valid_dataset, args, result_filename=None):
    """
    Training loop cho QA model với paper hyperparameters
    """
    ...
    with open(result_filename, 'w') as f:
        f.write('TEMPOQR PAPER REPRODUCTION\n')
        f.write('Model %s\n' % args.model)
        f.write('Supervision %s\n' % args.supervision)
        f.write('Learning rate %f\n' % args.lr)
        f.write('Batch size %d\n' % args.batch_size)
        f.write('Max epochs %d\n' % args.max_epochs)
        f.write('Total train samples %d\n' % len(dataset.data))
        f.write('Total valid samples %d\n' % len(valid_dataset.data))
        f.write('Fuse %s\n' % args.fuse)
        f.write('Frozen %d\n' % args.frozen)
        f.write('LM frozen %d\n' % args.lm_frozen)
        ...
        f.write('Early stopping patience %d\n' % args.patience)
    
    print(f'🎯 TEMPOQR PAPER REPRODUCTION')
    print(f'📊 Training samples: {len(dataset.data)}')
    print(f'📊 Validation samples: {len(valid_dataset.data)}')
    ...
    for epoch in range(args.max_epochs):
        ...
        if (epoch + 1) % args.valid_freq == 0:
            ...
            if eval_score > max_eval_score:
                ...
                torch.save(qa_model.state_dict(), checkpoint_file_name)
                max_eval_score = eval_score
                best_epoch = epoch + 1
                no_improvement_count = 0
                print(f'🏆 New best score: {max_eval_score:.4f} at epoch {epoch+1}')
            else:
            ...
            if no_improvement_count >= args.patience:
                print(f'⏹️ Early stopping triggered after {no_improvement_count} epochs without improvement')
                print(f'🏆 Best score: {max_eval_score:.4f} at epoch {best_epoch}')
                break
    
    print(f'🎉 Training completed!')
    print(f'🏆 Best validation score: {max_eval_score:.4f} at epoch {best_epoch}')
    print(f'📁 Best model saved: {checkpoint_file_name}')
...
# Tạo result filename
result_filename = 'results/{dataset_name}/{model_file}_paper.log'.format(
    dataset_name=args.dataset_name,
    model_file=args.save_to
)
...
print('🎉 Paper reproduction training finished!')
print('📁 Check results at:', result_filename)
print('📁 Check model at:', f'models/models/{args.dataset_name}/qa_models/{args.save_to}.ckpt')
print('\n📊 Expected Paper Performance:')
print('   Hits@1: ~0.46')
print('   Hits@10: ~0.67')
print('   Total entities: 125,726')
print('   Training time: ~12-24 hours on GPU')
