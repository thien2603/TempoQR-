...
def test_model_with_real_data():
    ...
    tkbc_model = loadTkbcModel(os.path.join(project_root, f'models/models/{dataset_name}/kg_embeddings/tcomplex.ckpt'))
    ...
    args.tkg_file = os.path.join(project_root, f'data/data/{dataset_name}/kg/train')
    if not os.path.exists(args.tkg_file):
        alternatives = ['train_corr20.txt', 'train_corr33.txt', 'train_corr50.txt', 'full.txt']
        for alt in alternatives:
            alt_path = os.path.join(project_root, f'data/data/{dataset_name}/kg/{alt}')
    ...
    model_path = os.path.join(project_root, f'models/models/{dataset_name}/qa_models/tempoqr_full_export.pt')
    ...
    test_indices = random.sample(range(len(dataset)), min(100, len(dataset)))
