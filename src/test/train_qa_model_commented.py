...
from src.core.qa_baselines import QA_lm, QA_embedkgqa, QA_cronkgqa
from src.core.qa_datasets import QA_Dataset_Baseline, QA_Dataset_TempoQR
from src.core.utils import print_info, getAllDicts
...
def set_seed(seed=42):
    ...
    - torch.cuda.manual_seed: PyTorch GPU random
    - torch.backends.cudnn.deterministic: Đảm bảo deterministic algorithms
    """
    random.seed(seed)
    np.random.seed(seed)
...
def eval(qa_model, dataset, batch_size = 128, split='valid', k=10):
    ...
    Args:
        qa_model: Model cần đánh giá
        dataset: Dataset để đánh giá (train/valid/test)
        batch_size: Kích thước batch cho evaluation
        split: Tên split đang đánh giá
        k: Giá trị k cho Hits@k metric
    
    Returns:
        eval_score: Điểm số evaluation tốt nhất
        eval_log: Log chi tiết kết quả
    """
    num_workers = 0  # Disable multiprocessing để tránh memory issues trên Windows
    qa_model.eval()  # Chuyển sang evaluation mode (tắt dropout, batchnorm)
    eval_log = []
    print_numbers_only = False
    k_for_reporting = k  # Giữ tên parameter vì có thể được dùng ở nơi khác
    k_list = [1,10]  # Đánh giá Hits@1 và Hits@10 (standard QA metrics)
    max_k = max(k_list)
    ...
    topk_answers = []  # Lưu top-k predictions cho mỗi câu hỏi
    total_loss = 0
    loader = tqdm(data_loader, total=len(data_loader), unit="batches")

    # Evaluation loop
    for i_batch, a in enumerate(loader):
        # Nếu size của split là bội số của batch size, cần break để tránh duplicate
        if i_batch * batch_size == len(dataset.data):
            break
        
        answers_khot = a[-1]  # Target answers (one-hot encoded, phần cuối cùng của tuple)
        scores = qa_model.forward(a)  # Forward pass qua model
        
        # Lấy top-k predictions cho mỗi sample trong batch
        for s in scores:
            pred = dataset.getAnswersFromScores(s, k=max_k)
            topk_answers.append(pred)
        
        # Tính loss (BCE loss cho multi-label classification)
    ...
    # Tính Hits@k cho từng giá trị k trong k_list
    eval_accuracy_for_reporting = 0
    for k in k_list:
        hits_at_k = 0
        total = 0
        question_types_count = defaultdict(list)  # Performance theo từng loại câu hỏi
        simple_complex_count = defaultdict(list)  # Performance theo simple/complex
        entity_time_count = defaultdict(list)  # Performance theo entity/time answers

        # Duyệt qua tất cả câu hỏi để tính accuracy
        for i, question in enumerate(dataset.data):
            ...
            predicted = topk_answers[i][:k]  # Top-k predictions
            
            # Kiểm tra xem có answer nào trong top-k không
            ...
            # Lưu kết quả theo từng category
        ...
        # Tính accuracy và log
        eval_accuracy = hits_at_k / total
        eval_accuracy_for_reporting = eval_accuracy  # Dùng giá trị lớn nhất (thường là Hits@10)
        
        eval_log.append('Hits at %d: %f' % (k, eval_accuracy))
        
        # Log chi tiết theo từng loại câu hỏi
...
def train(qa_model, dataset, valid_dataset, args, result_filename=None):
    ...
    Args:
        qa_model: Model cần train
        dataset: Training dataset
        valid_dataset: Validation dataset
        args: Command line arguments
        result_filename: File để log kết quả
    """
    num_workers = 0  # Disable multiprocessing để tránh memory issues
    ...
    os.makedirs('models/{dataset_name}/qa_models'.format(dataset_name=args.dataset_name), exist_ok=True)
    ...
    for epoch in range(args.max_epochs):
        qa_model.train()  # Chuyển sang training mode (bật dropout, batchnorm)
        epoch_loss = 0
        loader = tqdm(data_loader, total=len(data_loader), unit="batches")
...
def save_model(qa_model, filename):
    ...
    Args:
        qa_model: Model cần lưu
        filename: Đường dẫn file checkpoint
    """
...
def append_log_to_file(eval_log, epoch, result_filename):
    ...
    Args:
        eval_log: Log kết quả evaluation
        epoch: Epoch number
        result_filename: File log path
    """
...
# Load TKBC model (Temporal Knowledge Base Completion)
...
# Khởi tạo model và datasets dựa trên model type
...
# Final evaluation trên test set (commented out, có thể enable nếu cần)
