...
class QA_Dataset(Dataset):
    def __init__(self, 
        ...
        filename = os.path.join(project_root, 'data/data/{dataset_name}/questions/{split}.pickle'.format(
...
class QA_Dataset_TempoQR(QA_Dataset):
    ...
    def _collate_fn(self, items):
        ...
        input_ids = torch.from_numpy(self.pad_for_batch(tokenized_questions, self.tokenizer.pad_token_id, np.long)).long()
        ...
        entity_time_ids_padded = torch.from_numpy(entity_time_ids_padded).long()
        ...
        heads = torch.from_numpy(np.array([item[4] for item in items])).long()
        tails = torch.from_numpy(np.array([item[5] for item in items])).long()
        times = torch.from_numpy(np.array([item[6] for item in items])).long()
        start_times = torch.from_numpy(np.array([item[7] for item in items])).long()
        end_times = torch.from_numpy(np.array([item[8] for item in items])).long()
        tails2 = torch.from_numpy(np.array([item[9] for item in items])).long()
