...
def loadTkbcModel(tkbc_model_file):
    ...
    # Move to device based on config
...
def getAllDicts(dataset_name):
    ...
    base_path = os.path.join(project_root, 'data/data/{dataset_name}/kg/tkbc_processed_data/{dataset_name}/'.format(
    ...
    ent2id = dicts['ent_id']
    ts2id = dicts['ts_id']
    file_ent = os.path.join(project_root, 'data/data/{dataset_name}/kg/wd_id2entity_text.txt'.format(
        dataset_name=dataset_name
    ))
    file_rel = os.path.join(project_root, 'data/data/{dataset_name}/kg/wd_id2relation_text.txt'.format(
        dataset_name=dataset_name
    ))
    def readDict(filename):
        ...
        for encoding in encodings:
            try:
                ...
                continue
        raise ValueError(f"Could not read {filename} with any encoding: {encodings}")
