...
class QA_TempoQR(nn.Module):
	...
	def invert_binary_tensor(self, tensor):
		# Fix: Use logical_not() instead of subtraction for bool tensors
		inverted = torch.logical_not(tensor).float()
		return inverted
	...
	def forward(self, a):
		...
		#TKG embeddings
		head_embedding = self.entity_time_embedding(heads.long())
		...
		#Hard Supervision
		t1_emb = self.tkbc_model.embeddings[2](t1.long()) 
		t2_emb = self.tkbc_model.embeddings[2](t2.long()) 

        
        
		#entity embeddings to replace in sentence
		entity_time_embedding = self.entity_time_embedding(entities_times_padded.long())
		...
		indices_for_position_embedding = torch.from_numpy(v).to(combined_embed.device).long()
		...
		mask2 = ~(question_attention_mask.bool()).to(question_attention_mask.device)
