...
parser.add_argument('--save_to', default='tempoqr_quick', type=str,
...
def eval(qa_model, dataset, batch_size = 32, split='valid', k=10):
    """
...
def train(qa_model, dataset, valid_dataset, args, result_filename=None):
    """
    Training loop cho QA model với toàn bộ data
    """
    ...
    print(f'🚀 QUICK TEST MODE - FULL DATA')
...
def save_model(qa_model, filename):
    """
    Lưu model checkpoint - Fixed version
    """
    print('💾 Saving model to', filename)
    try:
        # Try saving state_dict directly
        ...
        try:
            ...
            print('✅ Saved model with alternative method')
...
# Tạo result filename
result_filename = 'results/{dataset_name}/{model_file}_quick_full.log'.format(
    dataset_name=args.dataset_name,
    model_file=args.save_to
)
...
print('🎉 Quick test with full data finished!')
