#!/usr/bin/env python3
"""
Complete test script for TempoQR Model with 10 custom questions
Based on evaluate_full_dataset.py structure with full model loading
"""
...
def test_model_complete():
    """Test model with 10 custom questions - complete implementation"""
    print("🧪 TempoQR Model Test - Complete Version")
    print("=" * 60)
    ...
    try:
        ...
        print(f"❌ Error importing torch/numpy: {e}")
        print("Please install required packages: pip install torch numpy")
        return
    ...
    try:
        ...
        print(f"Success rate: {(successful_predictions/total_questions)*100:.1f}%")
        print(f"Entity mappings loaded: {len(entity_mappings)}")
        ...
        if successful_predictions == total_questions:
            ...
            print(f"\n⚠️ {total_questions - successful_predictions} questions had issues")
